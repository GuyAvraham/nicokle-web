...
add_image_size( 'swell_project_thumb', 413, 232, true );
...
if ( ! function_exists( 'swell_scripts_n_styles' ) ) {
	function swell_scripts_n_styles() {
		...
		// 2.1.1  CSS
		wp_enqueue_style( 'swell-magnific-popup', get_template_directory_uri().'/css/magnific-popup.css', '', '1.0.0' );
        ...
        wp_enqueue_script( 'jquery' );
				...
				wp_enqueue_script( 'swell-popup', get_template_directory_uri() . '/js/jquery.magnific-popup.min.js', array( 'jquery' ), '1', true );	

        wp_enqueue_script( 'swell-flexslider', get_bloginfo( 'template_url' ).'/js/jquery.flexslider-min.js', array( 'jquery' ), '2.6.0', true);

        wp_enqueue_script( 'swell-wait-for-images', get_bloginfo( 'template_url' ).'/js/jquery.waitforimages.min.js', array( 'jquery' ), '1.0', true);
		...
		if( is_front_page() || is_page_template( 'home-page.php' )) {
			...
			wp_enqueue_script( 'swell-jquery-visibility', get_bloginfo( 'template_url' ).'/js/jquery-visibility.min.js', array( 'jquery' ), '1.0.12', true );
			wp_enqueue_script( 'swell-YTVideo', get_bloginfo( 'template_url' ).'/js/jquery.mb.YTPlayer.js', array( 'jquery' ), '0.1', true );
...
if ( ! function_exists( 'swell_video_embed' ) ) {
	function swell_video_embed( $link ) {
		...
		switch ( $video['type'] ) {
            ...
            case 'youtube': ?>
                <div id="youtube<?php echo $ttrust_config['video_counter']; ?>" class="video-container player youtube<?php echo get_theme_mod( 'swell_focus_pause',1 ) ? ' pause-on-lose-focus' : ''; ?>" style="display:block; margin: auto; background: rgba(0,0,0,0.5)"
                     data-property="{videoURL:'<?php echo $video['url']; ?>', containment:'#youtube<?php echo $ttrust_config['video_counter']++; ?>', mute:true,	autoPlay:true, loop:true, opacity:1 }"></div><?php
                break;

            case 'vimeo': ?>
                <div class="video-container">
                    <iframe class="vimeo player<?php echo get_theme_mod( 'swell_focus_pause', 1 ) ? ' pause-on-lose-focus' : ''; ?>" id="vimeo<?php echo $ttrust_config['video_counter']; ?>" src="<?php echo $video['url']; ?>?api=1&loop=1&portrait=0&controls=0&showinfo=0&autohide=1&rel=0&wmode=transparent&output=embed&player_id=vimeo<?php echo $ttrust_config['video_counter']++; ?>" width="100%" height="100%" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>
                </div><?php

				break;

				case 'self': ?>
					<div class="video-container self" style="width: 100%; height: 100%;" >
						<video id="self<?php echo $ttrust_config['video_counter']; ?>" class="html5-video player self<?php echo get_theme_mod( 'swell_focus_pause', 1 ) ? ' pause-on-lose-focus' : ''; ?>" width="100%" height="100%" loop autoplay muted>
...
if(! function_exists( 'swell_pullquotes' ) ) {
	function swell_pullquotes( $atts, $content = null ) {
		...
		if( $atts['side'] != 'right' || $atts['side'] != 'left' ) {

			return false;

		} else {

			return '<span class="pull pull' . $atts['side'] . '">' . do_shortcode( $content ) . "</span>";
